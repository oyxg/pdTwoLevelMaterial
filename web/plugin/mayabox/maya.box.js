...
//   更新：
//              2013/02/04 - 1.4
//		 @新增了一个maxthon皮肤@增加了一个在关闭box之前的回调函数@修改按钮的标签input为span标签。
//              2013/03/04 - 1.5
//		 @position属性新增加了一个"custom"自定义位置的选项，opt参数名为“custom”，custom的参数如同为｛x : 220，y : 160｝
//              2013/04/10 - 1.6
//		 @将dragBg绑定的事件换成了document以解决在拖动窗口的时候会出现选择内容的情况
...
// ========================
Maya.isIE6 = function() {
	return navigator.userAgent.toLowerCase().indexOf("msie 6") > 0 ? true
			: false;
}
...
Maya.CssElement = function(eleName, eleCSS) {
	for (p in eleCSS) {
		// console.info(eleName.className+"： "+p+"："+eleCSS[p]);
...
Maya.Box = function(cfg) {
	...
	this.minimize = function() {
		...
		if (opt.effect) {
			...
			Maya.CssElement(cp, {
				...
				top : parseInt(Maya.CssGet(contain).top) + winScreen.scrollTop
						+ "px"
			});
		...
		Maya.BindEvent(__tasker, "click", function() {
			if (opt.effect) {
				$(contain).slideDown(100);
				$(__tasker).hide(100, function() {
					$(this).remove();
				});
			} else {
				Maya.CssElement(contain, {
	...
	this.maximize = function() {
		...
		if (containStatus == "minimized" || containStatus == "normal") {
			...
			var l = winScreen.scrollLeft, t = winScreen.scrollTop;
			w = opt.width, h = opt.height - eleParam.boxBorHeight
					- eleParam.headHeight - eleParam.wrapperBorHeight
					- eleParam.btnsHeight;
		} else {
			containStatus = "minimized";
			var l = saveLeft, t = saveTop;
			w = saveWidth, h = saveHeight;
		}
		if (opt.effect) {
			$(contain).animate({
				left : l + "px",
	...
	var getScreenInfo = function() {
		winScreen.height = Math.max(win.document.documentElement.clientHeight,
				win.document.documentElement.scrollHeight);
		winScreen.width = Math.max(win.document.documentElement.clientWidth,
				win.document.documentElement.scrollWidth);
		winScreen.clientWidth = win.document.documentElement.clientWidth == 0 ? win.document.body.clientWidth
				: win.document.documentElement.clientWidth;
		winScreen.clientHeight = win.document.documentElement.clientHeight == 0 ? win.document.body.clientHeight
				: win.document.documentElement.clientHeight;
		winScreen.scrollLeft = Math.max(
				win.document.documentElement.scrollLeft,
				win.document.body.scrollLeft);
		winScreen.scrollTop = Math.max(win.document.documentElement.scrollTop,
	...
	var eventDragBgMouseUp = function(e) {
		...
		Maya.UnEvent(win.document, "mousemove", eventDragBgMouseMove);
		Maya.UnEvent(win.document, "mouseup", eventDragBgMouseUp);
	}
	var eventIframeLoad = function(e) {
		removeLoading();
		if (opt.iframeAuto) {
			if (Maya.isIE()) {
				var h = win.frames[iframeName].document.body.scrollHeight;
			} else {
				var h = win.frames[iframeName].document.documentElement.scrollHeight;
			}
			if(h>winScreen.clientHeight){
				h=winScreen.clientHeight-60;
			}
			Maya.CssElement(content, {
				height : h + "px"
			});
			...
			if (opt.position == "center") {
				var ch = h + eleParam.boxBorHeight + eleParam.headHeight
						+ eleParam.wrapperBorHeight + eleParam.btnsHeight;
	...
	var inlineLoad = function() {
		...
		if (opt.inlineAuto) {
			if (opt.position == "center") {
				var t = parseInt((winScreen.clientHeight - h
						- eleParam.boxBorHeight - eleParam.headHeight
						- eleParam.wrapperBorHeight - eleParam.btnsHeight) / 2);
				t = t < 0 ? 0 : t;
				if (opt.effect) {
					$(contain).animate({
						top : t + "px"
	...
	var onCloseBefore = function() {
		if (typeof (opt.onCloseBefore) == "function") {
			opt.onCloseBefore();
		}
	}
	var createElement = function() {
		...
		contain.onselectstart = function() {
			return false;
		}
		Maya.CssElement(contain, {
			width : opt.width + "px",
		...
		Maya.BindEvent(contain, "mousedown", function() {
			_self.setIndex(this);
		});
		...
		head.innerHTML = '<div class="' + opt.skin
				+ '_head_contain"><div class="' + opt.skin + '_head_text">'
				+ opt.text + '</div></div>';
		...
		if (opt.allowDrag) {
			// head单击事件
			Maya.BindEvent(head, "mousedown", function(e) {
				getScreenInfo();
				isHeadDown = true;
				if (Maya.isIE()) {
					e = win.event;
				...
				Maya.BindEvent(win.document, "mousemove", eventDragBgMouseMove);

				// 拖动层 鼠标弹起事件
				Maya.BindEvent(win.document, "mouseup", eventDragBgMouseUp);
			})
		}
		...
		if (opt.btnMaximize) {
			...
			Maya.BindEvent(btnMaximize, "click", function() {
				_self.maximize();
			});
		...
		if (opt.btnFold) {
			...
			Maya.BindEvent(btnFold, "click", function() {
				_self.fold();
			});
		...
		if (opt.btnMinimize) {
			...
			Maya.BindEvent(btnMinimize, "click", function() {
				_self.minimize();
			});
		...
		if ((opt.btns) != null) {
			...
			for ( var i = 0; i < opt.btns.length; i++) {
				(function(s) {
					var cname=(opt.btns[s].color==undefined) ? "" : opt.btns[s].color; 
					var btn = Maya.CreateElement(win, "button", {
						className : opt.skin + "_btns_btn "+cname
					});
					btns.appendChild(btn);
					btn.innerHTML = "<b>" + opt.btns[s].text + "</b>";
					// 如果是取消按钮
					if (opt.btns[s].isCancel) {
						Maya.BindEvent(btn, "click", function() {
							_self.close();
						});
	...
	var setContentHeight = function() {
		...
		eleParam.boxBorHeight = eleParam.containHeight - eleParam.headHeight
				- eleParam.wrapperHeight;
		eleParam.wrapperBorHeight = eleParam.wrapperHeight
				- eleParam.btnsHeight;
		eleParam.contentHeight = opt.height - eleParam.boxBorHeight
				- eleParam.headHeight - eleParam.btnsHeight
				- eleParam.wrapperBorHeight;
		Maya.CssElement(content, {
	...
	var renderLayout = function() {
		...
		if (typeof (opt.position) == "string") {
			...
			case "custom":
				var pos = {
					left : opt.custom.x + "px",
					top : opt.custom.y + "px"
				}
				break;
			case "center":
				var pos = {
					left : parseInt((winScreen.clientWidth - opt.width) / 2)
							+ winScreen.scrollLeft + "px",
					top : parseInt((winScreen.clientHeight - contain.offsetHeight) / 2)
							+ winScreen.scrollTop + "px"
				}
				break;
			case "centerTop":
				var pos = {
		...
		if (opt.effect && !Maya.isIE()) {
			...
			$(contain).slideDown(100, function() {
				setContent();
			});
		} else {
			setContent();
	...
	var setContent = function() {
		...
		if (opt.position == "center") {
			var t = parseInt((winScreen.clientHeight - opt.height
					- eleParam.boxBorHeight - eleParam.headHeight
					- eleParam.wrapperBorHeight - eleParam.btnsHeight) / 2);
			t = t < 0 ? 0 : t;
			Maya.CssElement(contain, {
	...
	var resetOpt = function() {
		if (new String(opt.width).indexOf("%") > 0) {
			opt.width = parseInt(winScreen.clientWidth * parseInt(opt.width)
					/ 100);
		}
		if (new String(opt.height).indexOf("%") > 0) {
			opt.height = parseInt(winScreen.clientHeight * parseInt(opt.height)
					/ 100);
		}
	}
